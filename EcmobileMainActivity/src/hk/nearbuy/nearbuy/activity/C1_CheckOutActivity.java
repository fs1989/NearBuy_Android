...
//import com.insthub.nearbuy.fragment.MyDialogFragment;
import hk.nearbuy.nearbuy.model.OrderModel;
import hk.nearbuy.nearbuy.model.ShoppingCartModel;
import hk.nearbuy.nearbuy.protocol.ApiInterface;
import hk.nearbuy.nearbuy.protocol.BONUS;
import hk.nearbuy.nearbuy.protocol.ORDER_INFO;
import hk.nearbuy.nearbuy.protocol.PAYMENT;
import hk.nearbuy.nearbuy.protocol.SHIPPING;
import hk.nearbuy.nearbuy.protocol.STATUS;
import hk.nearbuy.nearbuy.protocol.flowcheckOrderResponse;
...
//	private MyDialogFragment newFragment;
	...
	private LinearLayout delivery_time;
	...
	private final static int REQUEST_TIME = 11;
...
//	}
	...
	public void onClick(View v) {
		...
		case R.id.delivery_time:
			intent = new Intent(this, C4_DeliveryTimeActivity.class);
...
//			showDialog();
		...
		case R.id.balance_redPaper:
			try {
				...
				if (bonus.equals("1")) {
					...
					Resources resourc = (Resources) getBaseContext()
							.getResources();
					String not = resourc
							.getString(R.string.not_support_a_red_envelope);
					String log_str = resourc
							.getString(R.string.crash_log_analysis);
					ToastView toast2 = new ToastView(C1_CheckOutActivity.this,
							not);
		...
		case R.id.balance_submit:
			...
			if (null == payment) {
				ToastView toast1 = new ToastView(C1_CheckOutActivity.this,
						resourc.getString(R.string.warn_no_pay));
			...
			if (null == shipping) {
				ToastView toast1 = new ToastView(C1_CheckOutActivity.this,
						resourc.getString(R.string.warn_no_shipping));
				toast1.setGravity(Gravity.CENTER, 0, 0);
				toast1.show();
				return;
			}
			...
			if (checkCashOnDeliverOk(payment, shipping)) {
				if (null != selectedBONUS) {
					shoppingCartModel.flowDone(payment.pay_id,
							shipping.shipping_id, selectedBONUS.bonus_id,
							scoreNum, inv_type + "", inv_payee, inv_content
									+ "");
				} else {
					String comment = "指定配送时间: " +  time + "\n" + inv_payee;
					shoppingCartModel.flowDone(payment.pay_id,
							shipping.shipping_id, null, scoreNum,
							inv_type + "", comment, inv_content + "");
				}
	...
	public void setInfo() {
		...
		for (int i = 0; i < shoppingCartModel.balance_goods_list.size(); i++) {
			View view = LayoutInflater.from(this).inflate(
					R.layout.c1_check_out_body_item, null);
			TextView goods_name = (TextView) view
					.findViewById(R.id.body_goods_name);
			TextView goods_num = (TextView) view
					.findViewById(R.id.body_goods_num);
			TextView goods_total = (TextView) view
					.findViewById(R.id.body_goods_total);
			goods_name
					.setText(shoppingCartModel.balance_goods_list.get(i).goods_name);
			goods_num.setText("X "
					+ shoppingCartModel.balance_goods_list.get(i).goods_number);
			goods_total.setText("￥"
					+ shoppingCartModel.balance_goods_list.get(i).subtotal);
			body.addView(view);
			totalGoodsPrice += Float
					.valueOf(shoppingCartModel.balance_goods_list.get(i).subtotal);
		}
		...
		try {
			...
			int min_score = Math.min(Integer.valueOf(your_score),
	...
	public void OnMessageResponse(String url, JSONObject jo, AjaxStatus status)
		...
		if (url.endsWith(ApiInterface.FLOW_CHECKORDER)) {
			...
			if (payment.is_cod.equals("1")) {
				ToastView toast1 = new ToastView(C1_CheckOutActivity.this,
						getString(R.string.check_orders));
				toast1.setGravity(Gravity.CENTER, 0, 0);
				toast1.show();
				finish();
			} else {
			...
			if (upop_tn != null && !"".equals(upop_tn)) {
				// 银联sdk支付
				UPPayAssistEx.startPayByJAR(C1_CheckOutActivity.this,
						PayActivity.class, null, null, upop_tn, UPPay_mMode);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_ADDRESS_LIST) {
			...
			if (data != null) {
				time = data.getStringExtra("DELIVERT_MESSAGE");
				delivery_message.setText(time);
			...
			if (data != null) {
				...
				scoreChangedMoneyFormated = data
						.getStringExtra("bonus_formated");
			...
			if (data != null) {
				...
				if (null != bonusJSONString) {
					try {
						...
						redPaper_name.setText(selectedBONUS.type_name + "["
								+ selectedBONUS.bonus_money_formated + "]");
						bonus_text.setText("-"
								+ selectedBONUS.bonus_money_formated);
						refreshTotalPrice();
					} catch (JSONException e) {
			...
			if (str.equalsIgnoreCase("success")) {
				...
				String exiten = resource
						.getString(R.string.continue_shopping_or_not);
				final MyDialog mDialog = new MyDialog(C1_CheckOutActivity.this,
						exit, exiten);
				mDialog.show();
				mDialog.positive.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						mDialog.dismiss();
						Intent it = new Intent(C1_CheckOutActivity.this,
								EcmobileMainActivity.class);
						startActivity(it);
						finish();
				...
				ToastView toast = new ToastView(C1_CheckOutActivity.this,
						getResources().getString(R.string.pay_failed));
				toast.setGravity(Gravity.CENTER, 0, 0);
				toast.show();
				Intent intent = new Intent(C1_CheckOutActivity.this,
						E4_HistoryActivity.class);
	...
	private void showAlipayDialog() {
		...
		dialog.show();
		LinearLayout alipayLayout = (LinearLayout) view
				.findViewById(R.id.alipay);
		LinearLayout alipayWapLayout = (LinearLayout) view
				.findViewById(R.id.alipay_wap);
